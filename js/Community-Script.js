...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    const handlePollVote = async (userId, pollId, selectedOption) => {
      ...
      if (previousVote) {
        const previousOptionRef = ref(
          database,
          `polls/${userId}/${pollId}/options/${previousVote}`
        );
        ...
        const previousOptionSpan = document.querySelector(
          `#option-${previousVote} .poll-votes`
        );
      ...
      const selectedOptionRef = ref(
        database,
        `polls/${userId}/${pollId}/options/${selectedOption}`
      );
      ...
      const selectedOptionSpan = document.querySelector(
        `#option-${selectedOption} .poll-votes`
      );
      ...
      unsubscribe = onValue(sortedPollsQuery, async (snapshot) => {
        ...
        allPolls.forEach((poll, index) => {
          ...
          const updatePollVotes = async (userId, pollId, option, span) => {
            const optionRef = ref(
              database,
              `polls/${userId}/${pollId}/options/${option}`
            );
    ...
    const handleNewsfeed = () => {
      ...
      btnProfile.style.textDecoration = "none";
      btnBlogs.style.textDecoration = "none";
      btnStory.style.textDecoration = "none";
      btnPolls.style.textDecoration = "none";
      ...
      if (newsfeed === "polls") {
        btnPolls.style.textDecoration = "underline";
        ...
        btnStory.style.textDecoration = "underline";
        ...
        if (profileNav === "pending") {
          btnProfilePosts.style.textDecoration = "none";
          btnProfilePending.style.textDecoration = "underline";
        } else {
          btnProfilePending.style.textDecoration = "none";
          btnProfilePosts.style.textDecoration = "underline";
        }

        btnProfile.style.textDecoration = "underline";
